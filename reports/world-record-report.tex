...
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[height=1.5cm]{logo_topshop.jpg}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[height=1.5cm]{logo_thg.jpg}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[height=1.5cm]{logo_dreamlab.jpg}
\end{minipage}
...
\begin{center}
\begin{tabular}{lcl}
\textbf{Total Assets Generated} & \quad & \textbf{138} images and videos \\
...
\textbf{Speedup} & \quad & \textbf{8$\times$} (36 assets) to \textbf{14$\times$} (138 assets)\textsuperscript{*} \\
...
On February 25, 2026, the IRIS autonomous campaign generation system attempted to set a world record for AI-generated fashion advertising. Starting from a single four-panel garment photograph---Look~6 from the Topshop SS26 collection---an autonomous AI agent swarm produced \textbf{138 campaign assets} in 96 minutes of wall-clock time (64 minutes of active generation), including 133 images and composites plus 5 branded cinematic fashion films.

Critically, this was achieved \textbf{from a standing start in 3 hours on-site at THG}---no pre-built workflows, no pre-existing assets, no creative brief. The human creative director \textbf{never opened a creative interface}. No GUI was used. No manual pipeline construction. No drag-and-drop workflow building. The operator spoke voice instructions; the AI agent swarm autonomously constructed every workflow from scratch---the multi-GPU pipeline configuration, ComfyUI workflow JSON files, API integrations, Nano Banana (a ComfyUI custom node wrapping Google's Gemini~2.5 Flash Image API) prompt engineering, Veo~3.1 video generation calls, programmatic compositing scripts, and quality assurance checks.

\textbf{ComfyUI was operated entirely via its HTTP API under agentic control}---the agents queued workflows, polled for results, and downloaded outputs programmatically. No human interacted with the ComfyUI web interface at any point. The same agentic API pattern was used for all cloud services: Gemini~2.5 Flash Image (via the Nano Banana custom node API), Veo~3.1 (via Google's Generative Language API), and programmatic PIL/Pillow compositing.

Everything---the pipeline architecture, asset generation, error recovery, documentation, and this formal report---was produced within the 3-hour on-site window using a mix of local GPU compute and cloud AI services, directed entirely by voice.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{138 assets} from 21 voice prompts (16 directing generation, 5 directing documentation and QA) totalling $\sim$5 minutes of human input
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Screenshot_20260225_124427.jpg}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Screenshot_20260225_124530.jpg}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Screenshot_20260225_124750.jpg}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Screenshot_20260225_124904.jpg}
...
\end{tcolorbox}
\textbf{Result:} Five branded Veo~3.1 fashion films queued via the Gemini Generative Language API, targeting the highest-quality scene riff images: Victorian greenhouse, neon cityscape, neon corridor emergence, infinity mirror room, and cherry blossom. A loadable ComfyUI workflow JSON (\texttt{veo3-branded-video.json}) and a Veo prompt library (\texttt{veo-branded-prompts.json}) were generated and committed.
...
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{base_hero.jpg}
    \caption*{\small Shot 01: Hero (Flux 2 Dev)}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{base_rain.jpg}
    \caption*{\small Shot 02: Rain}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{base_night.jpg}
    \caption*{\small Shot 05: Night}
\end{minipage}
\caption{Phase 1 base generation --- chrome mannequin in the reference dress, generated by Flux 2 Dev on dual RTX 6000 Ada GPUs via ComfyUI Multi-GPU}
\label{fig:base_gen}
\end{figure}
...
\begin{minipage}{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{base_hero.jpg}
    \caption*{\small Before: Flux 2 Dev raw generation}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{refined_hero.jpg}
    \caption*{\small After: Nano Banana refinement}
\end{minipage}
\caption{Phase 2 style refinement comparison --- Nano Banana (Gemini 2.5 Flash Image) enhances chrome reflections, fabric texture, and environmental atmosphere}
...
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{composited_hero_9x16.jpg}
    \caption*{\small 9:16 Portrait}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{composited_rain_1x1.jpg}
    \caption*{\small 1:1 Square}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{composited_brutalist_16x9.jpg}
    \caption*{\small 16:9 Landscape}
\end{minipage}
\caption{Phase 3 composited outputs with ``STYLE REIMAGINED / TOPSHOP SS26'' typography applied via PIL/Pillow programmatic text overlay --- 3 aspect ratios per shot for multi-platform deployment}
\label{fig:composited}
\end{figure}
...
\textbf{Engine:} Veo 3.1 via Google GenMedia custom nodes for ComfyUI\\
\textbf{Duration:} 5 minutes (10:45--10:50 UTC)\\
\textbf{Output:} 6 animated fashion films (8 seconds each)

Videos were generated via the \texttt{Veo3VideoGenerationNode} custom node in ComfyUI---part of the official Google GenMedia node suite---using a 7-layer prompting framework (camera + lens + subject + action + setting + lighting + style). A loadable ComfyUI workflow (\texttt{veo3-branded-video.json}) was generated by the agent swarm and committed to the repository, enabling one-click reproduction of any branded video.
...
The repose phase adds 28 assets to the campaign total, bringing the image/composite count to \textbf{133}. With the addition of 5 branded Veo~3.1 fashion films (Phase~7), the total deliverable count reaches \textbf{138 assets}---all generated within the 3-hour on-site window.

\subsection{Phase 7: Branded Fashion Films (Veo 3.1)}

\textbf{Engine:} Veo 3.1 via ComfyUI Google GenMedia custom nodes (\texttt{Veo3VideoGenerationNode})\\
\textbf{Model:} \texttt{veo-3.1-generate}\\
\textbf{Output:} 5 branded 8-second fashion films at 9:16 and 16:9 aspect ratios

Five of the highest-quality scene riff images were selected for cinematic video treatment, each using the 7-layer prompting framework:
...
\toprule
\textbf{Video} & \textbf{Aspect} & \textbf{Camera} & \textbf{Scene} \\
\midrule
Greenhouse & 9:16 & Dolly push (85mm f/1.4) & Victorian conservatory, chrysanthemums, golden light \\
Neon Cityscape & 9:16 & Lateral track (85mm) & Rain-slicked Tokyo rooftop, Blade Runner neon \\
Neon Emerge & 16:9 & Forward dolly (85mm) & Dark corridor, teal/blue neon bars, volumetric haze \\
Mirror Room & 16:9 & Orbit track (85mm) & Kusama-inspired infinity mirror room, LED orbs \\
Cherry Blossom & 9:16 & Upward tilt (85mm) & Falling petals, blush pink canopy, soft diffusion \\
\bottomrule
\end{tabularx}
\caption{Branded Veo 3.1 fashion films---all queued via ComfyUI API, not manual GUI operation}
\end{table}
...
\begin{minipage}{0.19\textwidth}
    \centering
    \includegraphics[width=\textwidth]{riff_greenhouse.jpg}
    \caption*{\tiny Greenhouse}
\end{minipage}\hfill
\begin{minipage}{0.19\textwidth}
    \centering
    \includegraphics[width=\textwidth]{riff_neon_cityscape.jpg}
    \caption*{\tiny Neon Cityscape}
\end{minipage}\hfill
\begin{minipage}{0.19\textwidth}
    \centering
    \includegraphics[width=\textwidth]{riff_cherry_blossom.jpg}
    \caption*{\tiny Cherry Blossom}
\end{minipage}\hfill
\begin{minipage}{0.19\textwidth}
    \centering
    \includegraphics[width=\textwidth]{riff_mirror_room.jpg}
    \caption*{\tiny Mirror Room}
\end{minipage}\hfill
\begin{minipage}{0.19\textwidth}
    \centering
    \includegraphics[width=\textwidth]{riff_cathedral.jpg}
    \caption*{\tiny Cathedral}
\end{minipage}
\caption{Source images for the 5 branded Veo~3.1 fashion films --- the highest-quality scene riffs selected for cinematic video treatment. Each 8-second film uses the 7-layer prompting framework (camera, lens, subject, action, setting, lighting, style).}
\label{fig:branded_sources}
\end{figure}

The ComfyUI workflow was constructed programmatically by the AI agent swarm using the \texttt{Veo3VideoGenerationNode}~$\rightarrow$~\texttt{SaveVideo} pipeline. A loadable workflow JSON was committed to the repository at \texttt{configs/workflows/veo3-branded-video.json}, enabling one-click reproduction. This follows the same agentic pattern established throughout: the human never opened ComfyUI; all workflow files were generated from scratch by the agents via the HTTP API.
...
5 branded Veo films complete & 12:25 & T+125m & 138 \\
\bottomrule
\end{tabularx}
...
Phase 7: Branded Films (Veo 3.1) & 5 & $\sim$5 & 1.0 \\
\midrule
\textbf{Cumulative} & \textbf{138} & \textbf{$\sim$69} & \textbf{2.0} \\
\bottomrule
\end{tabular}
...
With repose + branded films (138 assets) & $\sim$15+ hours & $\sim$69 min & \textbf{13$\times$} \\
...
\bottomrule
\end{tabular}
\caption{Voice prompt efficiency metrics. $\sim$5 minutes of human creative direction produced 138 campaign assets.}
\end{table}
...
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
Four loadable ComfyUI workflow JSON files were created---all generated from scratch by the AI agents, with no human interaction with the ComfyUI GUI:

\begin{itemize}[nosep]
    \item \texttt{flux2-multigpu-campaign.json}: Base generation with \texttt{UNETLoaderMultiGPU} (cuda:0), \texttt{CLIPLoaderMultiGPU} (cuda:1)
    \item \texttt{nano-banana-garment-reskin.json}: Garment reskinning via \texttt{LoadImage}~$\rightarrow$~\texttt{ImageBatch}~$\rightarrow$~\texttt{GeminiImageNode}
    \item \texttt{nano-banana-repose.json}: Mannequin repose using dual-image input for pose matching
    \item \texttt{veo3-branded-video.json}: Branded video generation via \texttt{Veo3VideoGenerationNode}~$\rightarrow$~\texttt{SaveVideo}
\end{itemize}

All four workflows are loadable in ComfyUI for re-use. Agents controlled ComfyUI entirely via its HTTP API (\texttt{/prompt}, \texttt{/history}), queuing workflows, polling completion, and downloading outputs programmatically.
\end{minipage}
...
\begin{enumerate}[nosep]
    \item \textbf{Veo 3.1 API:} The \texttt{generateVideo} endpoint returns 404; use \texttt{predictLongRunning} instead. Video downloads require \texttt{-L} flag for HTTP redirects. The ComfyUI \texttt{Veo3VideoGenerationNode} (Google GenMedia suite) requires Comfy.org authentication; direct Gemini API calls via \texttt{predictLongRunning} provide an alternative route for agentic control.
    ...
    \item \textbf{Parallel Agent Throughput:} 5 simultaneous agents achieve 3.8--4.7 images/minute peak rate (4.7 during Phase~6 repose with 3 agents).
\end{enumerate}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Traditional baseline is a single-operator estimate.} The 4-hour / 15-minute-per-asset comparison is based on one operator's experience. Extrapolations to 138 assets assume no batch efficiency gains.
\end{enumerate}
...
\begin{itemize}[nosep]
    \item \textbf{138 assets} (133 images/composites + 5 branded films) generated from garment photographs in 96 minutes
...
\textbf{Continuous Improvement through Agentic Memory.} IRIS maintains persistent agentic memory via VisionFlow's ontology-backed knowledge graph (900+ ontology classes, 100+ CUDA-accelerated kernels). Each campaign session contributes to the system's evolving understanding of brand guidelines, garment categories, composition patterns, and technical discoveries. This means that with continued corporate use---across seasons, brands, and product lines---IRIS delivers progressively better results as its ontologies encode domain-specific knowledge: preferred camera angles for different garment types, colour palette affinities, compositing rules, and quality thresholds. The system learns from every interaction, making each subsequent campaign faster and more aligned with the brand's creative vision.
